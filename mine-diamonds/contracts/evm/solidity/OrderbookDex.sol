...
import {IInverseProjected1155} from "@paima/evm-contracts/contracts/token/IInverseProjected1155.sol";
...
contract OrderbookDex is IOrderbookDex, ERC1155Holder, ReentrancyGuard {
    ...
    /// @inheritdoc IOrderbookDex
    function createSellOrder(uint256 assetId, uint256 assetAmount, uint256 pricePerAsset)
        public
        virtual
        returns (uint256)
    {
        ...
        IInverseProjected1155(asset).safeTransferFrom(msg.sender, address(this), assetId, assetAmount, bytes(""));
    ...
    /// @inheritdoc IOrderbookDex
    function fillOrdersExactEth(uint256 minimumAsset, uint256[] memory orderIds) public payable virtual nonReentrant {
        ...
        for (uint256 i; i < length; ++i) {
            ...
            IInverseProjected1155(asset).safeTransferFrom(
                address(this), msg.sender, order.assetId, assetsToBuy, bytes("")
            );
    ...
    /// @inheritdoc IOrderbookDex
    function fillOrdersExactAsset(uint256 assetAmount, uint256[] memory orderIds) public payable virtual nonReentrant {
        uint256 length = orderIds.length;
    ...
    function cancelSellOrder(uint256 orderId) public virtual {
        ...
        IInverseProjected1155(asset).safeTransferFrom(address(this), msg.sender, order.assetId, assetAmount, bytes(""));
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return interfaceId == type(IOrderbookDex).interfaceId || super.supportsInterface(interfaceId);
    }
}
