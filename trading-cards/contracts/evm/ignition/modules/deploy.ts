...
const tradeModule = buildModule('TradeNft', m => {
  const result = createNftAndProxy(m);
  return {
    tradeNftContract: result.nftContract,
    tradeNftSaleProxyContract: result.nftSaleProxyContract,
  };
});

const packModule = buildModule('PackBuy', m => {
  // This address is the owner of the ProxyAdmin contract,
  // so it will be the only account that can upgrade the proxy when needed.
  // https://github.com/NomicFoundation/hardhat-ignition/issues/673
  const proxyAdminOwner = m.getAccount(0);

  const paymentContract = m.contract('GenericPayment', []);

  const paymentProxyContract = m.contract('GenericPaymentProxy', [
    paymentContract,
    proxyAdminOwner,
  ]);

  return { paymentProxyContract };
});
...
export default buildModule('deploy', m => {
  ...
  return {
    ...
    ...m.useModule(tradeModule),
    ...m.useModule(packModule),
  };
});
