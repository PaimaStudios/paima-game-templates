...
export class Game {
  ...
  public static import(data: string): Game {
    ...
    const tiles = gameData.map.tiles.map((t: Tile) => {
      ...
      if (t.owner) {
        ...
        if (t.unit) {
          const u = new Unit(owner, t.unit.type);
    ...
    const game = new Game(gameMap, players);
  ...
  private initMoves(player: Player) {
  ...
  public placeBuilding(
    ...
    action.newBuildingType = buildingType;
    action.goldDelta = Building.getPrice(buildingType) * -1;
    ...
    this.moves[this.turn].applyPlaceBuilding(this, action);
    return true;
  }

  // ACTION: create new unit
  public placeUnit(player: Player, target: Tile, unitType: UnitType) {
    const action: GameAction = new GameAction(
      'new_unit',
      target.getCoordinates(),
      null
    ...
    action.setTile(target);
    action.newUnitType = unitType;
    action.goldDelta = Unit.getPrice(unitType) * -1;
    ...
    this.moves[this.turn].applyPlaceUnit(this, action);
    return true;
  }

  // ACTION: move unit from tileA to tileB
  public moveUnit(player: Player, origin: Tile, target: Tile): boolean {
    const action: GameAction = new GameAction(
      'move',
      target.getCoordinates(),
    ...
    action.unitMoved = origin.unit?.id || -1;
    ...
    this.moves[this.turn].applyMoveUnit(this, action);
    return true;
  }
