...
export class RulesScreen extends ScreenUI {
  ...
  clickEventListener = (evt: MouseEvent) => {
    ...
    const mousePos = this.getMousePos(evt);
    const x = mousePos.x + this.offset_x;
    const y = mousePos.y + this.offset_y;
    this.buttonEvents.forEach(e => {
      if (
        x > e.coord.x &&
        x < e.coord.x + e.coord.width &&
        y > e.coord.y &&
        y < e.coord.y + e.coord.height
      ) {
        e.trigger();
      }
    });
